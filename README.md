...
```bash
crud-app-new$ sam build --use-container
```
...
```bash
crud-app-new$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
crud-app-new$ sam local start-api
crud-app-new$ curl http://localhost:3000/
```
...
```bash
crud-app-new$ sam logs -n HelloWorldFunction --stack-name "crud-app-new" --tail
```
...
```bash
crud-app-new$ pip install -r tests/requirements.txt --user
# unit test
crud-app-new$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
crud-app-new$ AWS_SAM_STACK_NAME="crud-app-new" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "crud-app-new"
```
...
>>>>>>> 53c7426 (Initial commit of serverless CRUD app using AWS SAM)
